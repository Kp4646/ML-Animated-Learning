...
def run_decision_tree_classification(data, max_depth=3, min_samples_split=2, criterion='gini'):
    ...
    try:
        ...
        # Generate tree visualization using a scikit-learn estimator for compatibility
        try:
            from sklearn.tree import DecisionTreeClassifier as SKDecisionTreeClassifier
            sk_clf = SKDecisionTreeClassifier(
                max_depth=max_depth,
                min_samples_split=min_samples_split,
                criterion=criterion,
                random_state=42
            )
            # Train sklearn copy for visualization only
            sk_clf.fit(X, y)
            tree_img = generate_tree_visualization(sk_clf, feature_names=['x1', 'x2'], class_names=np.unique(y))
        except Exception:
            # Fallback to using our model for visualization (may produce error image)
...
def run_decision_tree_regression(data, max_depth=3, min_samples_split=2):
    ...
    try:
        ...
        # Generate tree visualization using sklearn regressor for compatibility
        try:
            from sklearn.tree import DecisionTreeRegressor as SKDecisionTreeRegressor
            sk_reg = SKDecisionTreeRegressor(
                max_depth=max_depth,
                min_samples_split=min_samples_split,
                random_state=42
            )
            sk_reg.fit(X, y)
            tree_img = generate_tree_visualization(sk_reg, feature_names=['x1', 'x2'], regression=True)
        except Exception:
...
def predict_data_points(data, predict_points, max_depth=3, min_samples_split=2, criterion='gini'):
    ...
    try:
        ...
        # For visualization, train a sklearn copy to avoid compatibility issues with export_graphviz
        try:
            if is_regression:
                from sklearn.tree import DecisionTreeRegressor as SKDecisionTreeRegressor
                sk_model = SKDecisionTreeRegressor(max_depth=max_depth, min_samples_split=min_samples_split, random_state=42)
            else:
                from sklearn.tree import DecisionTreeClassifier as SKDecisionTreeClassifier
                sk_model = SKDecisionTreeClassifier(max_depth=max_depth, min_samples_split=min_samples_split, criterion=criterion, random_state=42)

            sk_model.fit(X, y)

            if is_regression:
                tree_img = generate_tree_visualization(sk_model, feature_names=['x1', 'x2'], regression=True)
                decision_boundary = generate_regression_surface(sk_model, X, y)
            else:
                tree_img = generate_tree_visualization(sk_model, feature_names=['x1', 'x2'], class_names=np.unique(y))
                decision_boundary = generate_optimized_decision_boundary(sk_model, X, y)
        except Exception:
            # Fallback to using our model (may return an error visualization)
            if is_regression:
...
def generate_tree_visualization(model, feature_names=None, class_names=None, regression=False):
    ...
    try:
        # Try using sklearn's plot_tree (no external dot dependency)
        try:
            from sklearn import tree as sktree
            import matplotlib.pyplot as plt
            print("generate_tree_visualization: attempting sklearn.plot_tree", flush=True)
            fig, ax = plt.subplots(figsize=(10, 6))
            sktree.plot_tree(model, feature_names=feature_names, class_names=class_names, filled=True, rounded=True, impurity=not regression, ax=ax)
            ...
            # If plot_tree isn't available or fails, fallback to dot flow
            print(f"plot_tree failed: {e_plot}", flush=True)
        ...
        max_depth_to_show = min(8, getattr(model, 'get_depth', lambda: 8)())  # Limit visualization depth
        ...
        # If Graphviz 'dot' is not available on PATH, avoid calling it and return a helpful image
        import shutil
        if shutil.which('dot') is None:
            msg = "Graphviz 'dot' executable not found on this system. Install Graphviz or ensure 'dot' is on PATH to enable tree visualizations."
