...
@app.route('/api/knn-upload-csv', methods=['POST'])
def knn_upload_csv():
    try:
        ...
        # Determine if CSV has header (defaults to true)
        header_flag = request.form.get('header', 'true').lower() == 'true'
        ...
        # Require at least 3 columns: x1, x2, y
        if df.shape[1] < 3:
            return jsonify({"error": "CSV must contain at least 3 columns: x1, x2, y"}), 400

        # Extract first two columns as features and third as label
        X = df.iloc[:, 0:2].values.tolist()
        y = df.iloc[:, 2].astype(str).tolist()
