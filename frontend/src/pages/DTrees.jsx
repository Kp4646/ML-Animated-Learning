...
import { checkHealth, uploadDTreesData, trainDTreesModel } from '../api';
...
function DTrees() {
  ...
  // Complex Data Mode States
  const [dataMode, setDataMode] = useState('interactive'); // 'interactive' or 'complex'
  const [complexStep, setComplexStep] = useState(0); // 0: Upload, 1: Select Vars, 2: Visualize
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const result = await uploadDTreesData(file);
  ...
  const handleTrainComplexModel = async () => {
    ...
    try {
      const result = await trainDTreesModel({
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              border: 'none',
              backgroundColor: dataMode === 'interactive' ? 'white' : 'transparent',
              color: dataMode === 'interactive' ? '#1f2937' : '#6b7280',
              fontWeight: dataMode === 'interactive' ? '600' : '400',
              boxShadow: dataMode === 'interactive' ? '0 1px 3px 0 rgba(0, 0, 0, 0.1)' : 'none',
              cursor: 'pointer',
              transition: 'all 0.2s'
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              borderRadius: '0.375rem',
              border: 'none',
              backgroundColor: dataMode === 'complex' ? 'white' : 'transparent',
              color: dataMode === 'complex' ? '#1f2937' : '#6b7280',
              fontWeight: dataMode === 'complex' ? '600' : '400',
              boxShadow: dataMode === 'complex' ? '0 1px 3px 0 rgba(0, 0, 0, 0.1)' : 'none',
              cursor: 'pointer',
              transition: 'all 0.2s'
            }}
          >
            ...
            <div style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '0.75rem', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
              ...
              <div style={{ marginTop: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1, paddingRight: '2rem' }}>
                  <h3 style={{ fontSize: '1rem', fontWeight: '600', marginBottom: '0.5rem' }}>Model Parameters</h3>
                  <div className="parameter-item" style={{ marginBottom: '1rem' }}>
                    <label style={{ display: 'block', marginBottom: '0.25rem' }}>Max Depth: {maxDepth}</label>
                    <input
                      type="range"
                      min="1"
                      max="15"
                  ...
                  <div className="parameter-item">
                    <label style={{ display: 'block', marginBottom: '0.25rem' }}>Criterion</label>
                    <select
                      value={criterion}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
                ...
                <div style={{ padding: '1rem', backgroundColor: '#f3f4f6', borderRadius: '0.5rem', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '1.1rem', fontWeight: '600', color: '#1f2937' }}>
                    {complexVisualization.type === '2D' ? '2D Scatter Plot' : 'PCA Projection (2D)'}
                  </div>
                </div>
              </div>

              {/* Reusing ComplexKNNVisualization component */}
              <ComplexKNNVisualization
                data={complexVisualization}
                isClassification={activeTab === 'classification'}
              />

              <div style={{ marginTop: '2rem', textAlign: 'center' }}>
                <button
                  onClick={resetComplexData}
                  style={{ padding: '0.75rem 1.5rem', backgroundColor: '#e5e7eb', color: '#374151', border: 'none', borderRadius: '0.375rem', cursor: 'pointer' }}
                >
                  Upload New Dataset
                </button>
              </div>
