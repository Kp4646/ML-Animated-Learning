...
export const uploadKnnCsv = async (file, hasHeader = true) => {
  try {
    const formData = new FormData();
    formData.append('file', file);
    formData.append('header', hasHeader ? 'true' : 'false');

    const response = await axios.post(`${API_URL}/knn-upload-csv`, formData, {
      headers: {
    ...
    console.error('Error uploading KNN CSV:', error);
    throw error.response?.data || error;
  }
};
